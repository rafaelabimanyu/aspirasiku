...
@section('title', 'Masuk - Aspirasiku')
    ...
    <div class="max-w-md w-full bg-white p-8 rounded-xl shadow-sm border border-slate-200">
        ...
        <form class="space-y-6" action="{{ route('login') }}" method="POST">
            ...
            <div>
                ...
                <div class="mt-1">
                    <input id="login" name="login" type="text" autocomplete="username" required value="{{ old('login') }}" class="appearance-none block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors">
                </div>
                @error('login')
